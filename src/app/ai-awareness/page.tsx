...
import { getAIAwarenessMetrics } from "@/utils/dataAggregation";
...
export default function AIAwarenessPage() {
  ...
  if (!data || data.length === 0 || !analytics) {
    return (
      <div className="flex flex-col gap-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">AI Awareness</h1>
          <p className="text-slate-500 mt-2">Adoption rates and awareness of Artificial Intelligence technologies among youth.</p>
        </div>
  ...
  const readinessScore = Math.round(analytics.aiReadinessIndex);
